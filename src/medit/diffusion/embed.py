...
from typing import Any, Dict, Mapping, Union  # <<< still fine, we use Union
...
# -------------------------------------------------------------------------
# Helpers to normalize config inputs
# -------------------------------------------------------------------------

# Diffusion config can be passed either as a dataclass or a plain mapping
DiffCfgLike = Union[DiffusionConfig, Mapping[str, Any]]  
...
# -------------------------------------------------------------------------
# High-level geometry API (uses DiffusionConfig directly)
# -------------------------------------------------------------------------

def build_eggfm_geometry_from_config( 
    ad: sc.AnnData,
    energy_model: EnergyMLP,
    diff_cfg: DiffCfgLike,             
    obsm_key: str,
) -> sc.AnnData:
    """
    High-level EGGFM geometry builder.

    - Always writes diffusion embedding into ad.obsm[obsm_key].
    - Always stores:
        - P (Markov kernel) in ad.uns[f"{obsm_key}_P"]
        - eigenvalues in ad.uns[f"{obsm_key}_eigvals"]
        - kNN distances/connectivities in ad.obsp
          (these are written by EGGFMDiffusionEngine.build_embedding)
    """
    ...
    # Engine now always returns (embedding, P, eigvals)
    embedding, P, eigvals = engine.build_embedding( 
        ad_prep=ad,
    )

    # Always store embedding
    ad.obsm[obsm_key] = embedding

    # Always stash kernel/eigvals into .uns, namespaced by obsm_key
    ad.uns[f"{obsm_key}_P"] = P
    ad.uns[f"{obsm_key}_eigvals"] = eigvals
